...
class TUNInterface : public SystemInterface<TunMessage>, public SyncronizedShutdown {
		...
		int tunnel_fd = 0;
		int interface_setup(const char *dev, int flags, const char *ip, const char *destination, const char *mask, int mtu, int tx_queuelen);
		...
		void interface_set_txqueuelen(const char *deviceName, int txQueueLen);
	public:
		TUNInterface();
